...
class GradingWindow(QDialog):
    ...
    def report_all_grades_for_assignment(self):
        ...
        if not data_in:
            self.query_status.setText("No grades for assignment: {}".format(self.assignment_id_input.text()))
            return
    ...
    def report_all_grades_for_student(self):
        ...
        if not data_in:
            self.query_status.setText("No grades for student: {}".format(self.student_id_input.text()))
            return
        for i in range(len(data_in)):
            data_in[i] = list(data_in[i])
        self.model.updateData(data_in)
    ...
    def check_student(self):
        if not self.student_id_input.text():
            self.query_status.setText("Student ID is empty")
            return False
    ...
    def check_assignment(self):
        if not self.assignment_id_input.text():
            self.query_status.setText("Assignment ID is empty")
            return False

        if not Grading().check_assignment(self.assignment_id_input.text()):
            self.query_status.setText("Assignment ID does not exist")
            return False
        return True
