...
class Registration:
    ...
    def check_course_exists(self, course_id) :
        mycursor.execute("SELECT * FROM course WHERE course_id = %s", (course_id,))
        myresult = mycursor.fetchall()
        if len(myresult) != 0:
            return True
        return False
    ...
    def get_student_name_from_id(self, student_id):
        mycursor.execute("SELECT person_id FROM student WHERE student_id = %s", (student_id,))
        myresult = mycursor.fetchall()
        person_id = myresult[0][0]
...
def main():
    ...
    reg.set_student_id(100)
    print(reg.check_student_info())
    print(reg.check_course_exists(100))
    print(reg.get_student_name_from_id(10))
